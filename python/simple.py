...
class Order:
    ...
    def __repr__(self):
        return f"<Order buy_or_sell={self.buy_or_sell.value} order_type={self.order_type} price={self.price} quantity={self.quantity} oco_id={self.oco_id}>"
...
class Agent:
    ...
    def flatten_all(self, row):
        if self.positions >= 0:
            market_order = Order(
                OrderSide.SELL, OrderType.MARKET, row.close, self.positions, None
            )
            self.fill_order(market_order, row)
        if self.positions <= 0:
            market_order = Order(
                OrderSide.BUY, OrderType.MARKET, row.close, -1 * self.positions, None
            )
            self.fill_order(market_order, row)
    ...
    def create_tailing_stop_order(
        self, buy_or_sell, stop_price, trail_pts, quantity, oco_id=None
    ):
        order = Order(
            buy_or_sell,
